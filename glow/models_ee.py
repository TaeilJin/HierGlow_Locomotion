...
class FlowNet(nn.Module):
    ...
    def addEndEffectorElement(self,z, ee_cond):
        ...
        for ii, index in enumerate(self.ee_idx):
            z[:,index,:] = ee_cond[:,ii,:] 
        return z

    def forward(self, z, cond, ee_cond, logdet=0., reverse=False, eps_std=None):
        if not reverse:

            masked_init = torch.zeros(z.shape).to(z.device)
            masked_init = self.addEndEffectorElement(masked_init,ee_cond)
            z_u_cond = torch.cat((cond,masked_init), dim =1)
            for layer in self.layers:
                z, cond, logdet = layer(z, z_u_cond, logdet, reverse=False)
            return z, logdet
        else:
            masked_init = torch.zeros(z.shape).to(z.device)
            masked_init = self.addEndEffectorElement(masked_init,ee_cond)
            ...
            for i,layer in enumerate(reversed(self.layers)):
                z, cond, logdet = layer(z, z_u_cond, logdet=0, reverse=True)
            return z
